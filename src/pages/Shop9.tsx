...
import { Search, ChevronLeft, ChevronRight, Eye, LayoutGrid, Rows } from "lucide-react";
...
export default function Shop9() {
  ...
  const [viewMode, setViewMode] = useState<"single" | "double">(() => {
    const saved = localStorage.getItem("shop9ViewMode");
  ...
  const toggleViewMode = () => {
    ...
    setViewMode(newMode);
    localStorage.setItem("shop9ViewMode", newMode);
  };
  ...
  }, [searchTerm, sortBy, currentPage, filters, maxPrice, setSearchParams]);
                ...
                {paginatedProducts.map((product) => (
                  <div key={product.id} className="relative group">
                    ...
                    <Button
                      size="sm"
                      variant="secondary"
                      className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity shadow-lg border-2"
